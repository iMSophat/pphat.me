{{-- <div id="playList">
    <div class="py-5 px-3">
        <div class="border rounded-md p-5 flex flex-wrap gap-3">
            <div class="w-full">
                <h1 class="font-medium text-base">Whereas recognition of the inherent dignity</h1>
                <label for="" class="text-xs uppercase text-slate-500">PPHAT</label>
            </div>
            @for ($i = 0; $i < 16; $i++)
                <button type="button" class="h-9 px-2 min-w-[2.25rem] flex items-center justify-center text-sm border rounded-md">{{ $i+1 }}</button>
            @endfor
        </div>
    </div>
</div> --}}
...
<div id="list">
    <ul class="py-5 flex flex-col gap-2 px-3">
        @for ($i = 0; $i < 40; $i++)
        <li class="flex gap-3 relative group cursor-pointer">
            <button class="absolute hidden transition-all group-hover:block rounded-md p-1 top-0 right-0">
